...
前两版都栽在同一件事上：**把敦煌当成贴图往通用列表上刷**。
v1.1 刷了颜色，v1.2 刷了纹样（藻井 / 佛龛 / 榜题），但骨架始终是
「标题栏 + 标签栏 + 搜索栏 + 一条等权重的竖列表」—— 所以看着还是旧。

v1.3 换的是**构图与层级**：

| 位置 | 做法 |
|---|---|
| **顶栏** | 中国红实块 + 同心圆藻井，标题 900 字重 + 硬偏移阴影（国潮的招牌手法：实色不渐变、硬边不柔和） |
| **可点元素** | 统一是「贴纸」：白底 + 2px 墨描边 + 2px 硬偏移阴影，按下时位移贴合阴影 |
...
**它就是列表里的第一行，只多一个 `.is-hero` 类。**

`pickHero()` 在渲染时挑出第一条「还在跑」的下载（进行中 / 已暂停 / 安全扫描，
被拦截的不算），`renderView()` 顺手 toggle 这个类。**DOM 一个节点都没多**，
更新路径和普通行完全一致 —— 内存与性能零代价。

同一个 DOM 之所以能长出两副样子，靠的是 `display: contents` 把 `l1 / l2 / bar-row`
三层摊平进 `.main`，再用 `order` 重排成单行；`.is-hero` 下把这些还原回块级，
于是同一个结构呈现出完全不同的层级。

### 进度为什么没有横条

横条被**佛龛贴金**取代了。进度写进行节点的 `--p` 变量，由 `.ic-box::before`
的 `scaleY(var(--p))` 驱动 —— 纯 CSS，**没有额外 DOM，没有额外定时器**。
一件下载完成，那尊龛正好镀满。
...
- 深色模式：墨底 + 米白描边，红与金保持不变（红金在墨底上更亮）；
  榜题会**反转成金底墨字**，否则米白底 + 金字看不清
- 扩展图标是藻井：土红底 + 两道金线方胜箍 + 正中一支向下的箭
...
- `tools/preview-idle.html` —— 没有下载时（主角卡退场）
- `tools/preview-empty.html` —— 空状态

三个预览都严格照 `createRow()` 的 DOM 层级写，所以 `display: contents` 那套
摊平逻辑能在这里被测到 —— 这一点很重要，之前就因为预览页结构没跟着改而误判过。
...
## 文件结构
    ...
    popup.css            「国潮」主题：红顶栏 / 贴纸胶囊 / 佛龛贴金 / 主角卡 / 深色模式
    ...
    tools/preview-idle.html   离线静态预览：没有下载时（主角卡退场）
