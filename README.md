...
| 当前版本 | **v1.4.0** · 清爽界面与 18 类文件图标；面板与设置页的版本号直接读取 `manifest.json` |
...
1. 下载 [v1.4.0 安装包 `joly-download-v1.4.0.zip`](https://github.com/JolyI/joly-download/releases/download/v1.4.0/joly-download-v1.4.0.zip)，解压到准备长期保留的目录
...
版本详情见 [v1.4.0 发布说明](docs/release-v1.4.0.md) 与 [更新日志](CHANGELOG.md)。
...
面板与设置页使用清爽的灰白底色、柔和蓝色与细边框，跟随系统切换为柔和深色。
减少粗描边、硬阴影和纹样，让文件信息与下载状态更容易阅读。

- **统一品牌图标**：工具栏使用透明底的 J 字母与下载箭头组合，提供 16 / 32 / 48 / 128 像素版本。
- **信息分行**：第一行显示文件名与状态，第二行显示来源站点、大小与时间；
  下载中第三行显示进度、速度和剩余时间，暂停时保留进度。
- **轻量进度条**：细横条由 `.bar-row` 的伪元素绘制，复用行上的 `--p` 变量，
  通过 `scaleX(var(--p))` 更新，不增加 DOM 或定时器。
- **当前任务弱强调**：`pickHero()` 仍为第一条进行中、已暂停或安全扫描的下载添加
  `.is-hero`，只用浅底色与细边框突出，保留与普通行一致的信息层级。
- **空状态**：使用简洁的下载线框图标。
- **交互与状态**：蓝色用于选中和焦点，已完成使用灰色，暂停用琥珀色，危险操作用红色；
  移除盖章等强装饰动效，只保留轻微的交互反馈，并尊重系统减少动态效果的设置。

### 文件图标

每个文件使用 **32 像素的浅色圆角底与居中符号**，在小面板里保持清楚、统一的视觉重量。
图片使用蓝色、表格使用绿色，其余格式也有对应的形状与配色；深色模式同步调整底色与前景色。
...
- `tools/preview-file-icons.html` —— 18 类文件图标，复用正式页面的 SVG 图形与样式

面板预览中的下载条目保持与 `createRow()` 一致的 DOM 层级，便于检查真实列表的分行布局与状态样式。
...
静态预览用于检查布局与亮暗主题；真实下载、系统通知、文件操作和进程内存需在加载扩展后验证。
...
以上数值是性能验收目标；v1.4.0 未重新实测进程内存，不将静态预览或脚本检查作为内存验收结果。
...
## 文件结构
    ...
    popup.css            灰白柔蓝主题：分行布局 / 细横进度条 / 当前任务弱强调 / 深色模式
